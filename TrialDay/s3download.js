...
console.log('Remember to fill the aws.config.json file with your credentials.');

if (process.argv.indexOf('-url') < 0 || process.argv.indexOf('-file') < 0 ) {
    console.error('Usage error: node s3download.js -bucketname <bucket name> -file <file/path/> [-downloads <concurrent downloads>]');
    return;
}
